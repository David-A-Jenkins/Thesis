...
error = c(0.2)
b_start = c(-0.5,1,1)
forget_par <- c(0.9958,0.958,0.895,0.58)
obsn_par <- c(1,10,25,100)
...
source("DM_sim_linear_1obperday_final.R") # DM.sim.linear.1
...
if(!(obsn_par[job_id]==1 & updaten_par[job_id]==1)){
  DM.sim.linear.N(iter=1000, sampsize=365, valsize = 365, 
                  filename1 = paste0("DM_sim_result_",obsn_par[job_id],"ob_",updaten_par[job_id],"up_1000_allT_forget",round(100*forget_par[job_id],0)), 
                  filename2 =  paste0("DM_sim_result_",obsn_par[job_id],"ob_",updaten_par[job_id],"up_1000_sumT_forget",round(100*forget_par[job_id],0)), 
                  b_change = c(0,0.2,0.5,1,2), error = c(0.2), b_start = c(-0.5,1,1), 
                  obsn = obsn_par[job_id], updaten = updaten_par[job_id], 
                  forget = forget_par[job_id], init_period=237
)
}
