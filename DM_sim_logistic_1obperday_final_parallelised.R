...
DM.sim.logistic.1 <- function(it, sampsize=365, valsize = 365,b_change1,b_change2,s1,s2,er,b_start,forget,job_id){
  library(dma)
  library(pROC)
  #library(plyr)
  ...
  assign(paste0("LP"), rowSums(cbind(cbind(intercept_dat,dat_full)[1:sampsize1,]*cbind(coefmat1,coefmat2,coefmat3),
                                            rnorm(sampsize1, mean = 0, sd = er))))
  
  Pi <- exp(LP)/(1+exp(LP))
  
  assign(paste0("ydat_full"), rbinom(sampsize1,1, Pi))
  ...
  assign(paste0("dma.test"), logistic.dma(eval(parse(text=(paste("dat",sep="")))),
                                 eval(parse(text=(paste("ydat",sep="")))),mmat1,lambda=forget,initialperiod=NULL)) # stops at sampsize
  
  
  assign(paste0("dma.test_cont"), logistic.dma(eval(parse(text=(paste("dat_full",sep="")))),
                                      eval(parse(text=(paste("ydat_full",sep="")))),mmat1,lambda=forget,initialperiod=NULL)) # stops at sampsize
  ...
  ### Predict LP in val data for each model
  lm_pred_lp <- lm_b1 + lm_b2*dat_val[,1] + lm_b3*dat_val[,2]
  dm_pred_lp <- dm_b1 + dm_b2*dat_val[,1] + dm_b3*dat_val[,2]
  dmc_pred_lp <- dm_b_con1 + dm_b_con2*dat_val[,1] + dm_b_con3*dat_val[,2]
  vc1_pred_lp <- vc1_b1 + vc1_b2*dat_val[,1] + vc1_b3*dat_val[,2]
  vc2_pred_lp <- vc2_b1 + vc2_b2*dat_val[,1] + vc2_b3*dat_val[,2]
  vc11_pred_lp <- vc11_b1 + vc11_b2*dat_val[,1] + vc11_b3*dat_val[,2]
  vc22_pred_lp <- vc22_b1 + vc22_b2*dat_val[,1] + vc22_b3*dat_val[,2]
  
  ### Predict outcome in val data for each model
  lm_pred_prob <- exp(lm_pred_lp) / (1+exp(lm_pred_lp))
  dm_pred_prob <- exp(dm_pred_lp) / (1+exp(dm_pred_lp))
  dmc_pred_prob <- exp(dmc_pred_lp) / (1+exp(dmc_pred_lp))
  vc1_pred_prob <- exp(vc1_pred_lp) / (1+exp(vc1_pred_lp))
  vc2_pred_prob <- exp(vc2_pred_lp) / (1+exp(vc2_pred_lp))
  vc11_pred_prob <- exp(vc11_pred_lp) / (1+exp(vc11_pred_lp))
  vc22_pred_prob <- exp(vc22_pred_lp) / (1+exp(vc22_pred_lp))
  ...
  ### Calibration slope
  cal_s_lm_mse <- glm(ydat_val ~ lm_pred_lp, family="binomial") 
  cal_s_dm_mse <- glm(ydat_val ~ dm_pred_lp, family="binomial")
  cal_s_dmc_mse <- glm(ydat_val ~ dmc_pred_lp, family="binomial")
  cal_s_vc1_mse <- glm(ydat_val ~ vc1_pred_lp, family="binomial")
  cal_s_vc2_mse <- glm(ydat_val ~ vc2_pred_lp, family="binomial")
  cal_s_vc11_mse <- glm(ydat_val ~ vc11_pred_lp, family="binomial")
  cal_s_vc22_mse <- glm(ydat_val ~ vc22_pred_lp, family="binomial")
  
  ### Calibration intercept
  cal_i_lm_mse <- glm(ydat_val ~ 1, offset = lm_pred_lp, family="binomial") 
  cal_i_dm_mse <- glm(ydat_val ~ 1, offset = dm_pred_lp, family="binomial")
  cal_i_dmc_mse <- glm(ydat_val ~ 1, offset = dmc_pred_lp, family="binomial")
  cal_i_vc1_mse <- glm(ydat_val ~ 1, offset = vc1_pred_lp, family="binomial")
  cal_i_vc2_mse <- glm(ydat_val ~ 1, offset = vc2_pred_lp, family="binomial")
  cal_i_vc11_mse <- glm(ydat_val ~ 1, offset = vc11_pred_lp, family="binomial")
  cal_i_vc22_mse <- glm(ydat_val ~ 1, offset = vc22_pred_lp, family="binomial")
  
  ### disc
  disc_lm_mse <- roc(ydat_val ~ lm_pred_prob, quiet = TRUE) 
  disc_dm_mse <- roc(ydat_val ~ dm_pred_prob, quiet = TRUE)
  disc_dmc_mse <- roc(ydat_val ~ dmc_pred_prob, quiet = TRUE)
  disc_vc1_mse <- roc(ydat_val ~ vc1_pred_prob, quiet = TRUE)
  disc_vc2_mse <- roc(ydat_val ~ vc2_pred_prob, quiet = TRUE)
  disc_vc11_mse <- roc(ydat_val ~ vc11_pred_prob, quiet = TRUE)
  disc_vc22_mse <- roc(ydat_val ~ vc22_pred_prob, quiet = TRUE)
  ...
  ###
  results1 <- cbind(it,1:valsize,lm_mse, dm_mse, dmc_mse, vc1_mse, vc2_mse, vc11_mse, vc22_mse,
  ...
  results2 <- t(c(it, as.numeric(cal_s_lm_mse$coefficients[2]), as.numeric(cal_s_dm_mse$coefficients[2]), as.numeric(cal_s_dmc_mse$coefficients[2]), as.numeric(cal_s_vc1_mse$coefficients[2]), as.numeric(cal_s_vc2_mse$coefficients[2]), as.numeric(cal_s_vc11_mse$coefficients[2]), as.numeric(cal_s_vc22_mse$coefficients[2]),
  ...
  write.table(results1,paste0("Results/logistic1/results1_b1_",b_change1,"_scen_",s1,"_b2_",b_change2,"_scen_",s2,"_sd_",er,"_iter_",it,".txt"), sep="\t", row.names=FALSE, col.names=FALSE)
  write.table(results2,paste0("Results/logistic1/results2_b1_",b_change1,"_scen_",s1,"_b2_",b_change2,"_scen_",s2,"_sd_",er,"_iter_",it,".txt"), sep="\t", col.names=FALSE)
